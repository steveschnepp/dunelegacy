...
void Game::initGame(const GameInitSettings& newGameInitSettings) {
    ...
    switch(gameInitSettings.getGameType()) {
        ...
        case GameType::CustomMultiplayer: {
            ...
            { // Scope
                INIMapLoader INIMapLoader(this, gameInitSettings.getFilename(), gameInitSettings.getFiledata());
            }
            ...
            if(bReplay == false && gameInitSettings.getGameType() != GameType::CustomGame && gameInitSettings.getGameType() != GameType::CustomMultiplayer) {
                /* do briefing */
                SDL_Log("Briefing...");
                BriefingMenu briefing(gameInitSettings.getHouseID(), gameInitSettings.getMission(),BRIEFING);
                briefing.showMenu();
            }
        } break;
            ...
            if(selectedList.size() == 1) {
                ...
                for(auto i = xPos; i < (xPos + structuresize.x); i++) {
                    for(auto j = yPos; j < (yPos + structuresize.y); j++) {
                        ...
                        auto drawLocation = calcDrawingRect(image, screenborder->world2screenX(i*TILESIZE), screenborder->world2screenY(j*TILESIZE));
    ...
    while(SDL_PollEvent(&event)) {
        ...
        if (pInGameMenu != nullptr) {
            ...
            switch (event.type) {
                ...
                case SDL_TEXTINPUT: {
                    if(chatMode) {
                        const auto newText = convertUTF8ToISO8859_1(event.text.text);
                    ...
                    for(int x = xPos; x < xPos + structuresize.x; x++) {
                        ...
                        if(pTile->hasANonInfantryGroundObject()) {
                            ...
                            if(pObject->isAUnit() && pObject->getOwner() == pBuilder->getOwner()) {
                                ...
                                const auto newDestination = currentGameMap->findDeploySpot(pUnit, Coord(xPos, yPos), tempRandomGen, pUnit->getLocation(), structuresize);
