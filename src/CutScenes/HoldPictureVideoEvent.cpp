...
HoldPictureVideoEvent::HoldPictureVideoEvent(SDL_Surface* pSurface, int numFrames2Hold, bool bFreeSurface, bool bCenterVertical)
{
    if(pSurface == nullptr) {
        ...
        auto pTmp = Scaler::defaultDoubleSurface(pSurface, bFreeSurface);
...
{
    if(pTexture != nullptr) {
        auto dest = calcAlignedDrawingRect(pTexture.get(), HAlign::Center, bCenterVertical ? VAlign::Center : VAlign::Top);
