...
BuilderList::BuilderList(Uint32 builderObjectID) {
    ...
    BuilderList::resize(BuilderList::getMinimumSize().x, BuilderList::getMinimumSize().y);
}
    ...
    if(pBuilder != nullptr) {
        ...
        for(const auto& buildItem : pBuilder->getBuildList()) {
            ...
            if((i >= currentListPos) && (i < currentListPos+getNumButtons(getSize().y) )) {
                ...
                // draw price
                {
                    auto pPriceTexture = pFontManager->createTextureWithText(fmt::sprintf("%d", buildItem.price), COLOR_WHITE, FONT_STD10);
                ...
                if(buildItem.num > 0) {
                    ...
                    auto pNumberTexture = pFontManager->createTextureWithText(fmt::sprintf("%d", buildItem.num), COLOR_RED, FONT_STD10);
