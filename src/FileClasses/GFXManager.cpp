...
GFXManager::Zoomable GFXManager::getObjPic(unsigned int id, int house) {
    if(id >= NUM_OBJPICS) {
        THROW(std::invalid_argument, "GFXManager::getObjPic(): Unit Picture with ID %u is not available!", id);
    }

    for(auto z = 0; z < NUM_ZOOMLEVEL; z++) {
        ...
        if(objPicTex[id][house][z] == nullptr) {
            ...
            if(id == ObjPic_Windtrap) {
                ...
                objPicTex[id][house][z] = convertSurfaceToTexture(generateWindtrapAnimationFrames(objPic[id][house][z].get()).release(), true);
            } else if(id == ObjPic_Terrain_HiddenFog) {
                const auto pHiddenFog = convertSurfaceToDisplayFormat(objPic[id][house][z].get(), false);
                ...
                objPicTex[id][house][z] = convertSurfaceToTexture(pHiddenFog.get(), false);
            } else if(id == ObjPic_CarryallShadow) {
                const auto pShadow = convertSurfaceToDisplayFormat(objPic[id][house][z].get(), false);
                ...
                objPicTex[id][house][z] = convertSurfaceToTexture(pShadow.get(), false);
...
SDL_Texture* GFXManager::getUIGraphic(unsigned int id, int house) {
    ...
    if(uiGraphicTex[id][house] == nullptr) {
        ...
        if(id >= UI_MapChoiceArrow_None && id <= UI_MapChoiceArrow_Left) {
            uiGraphicTex[id][house] = convertSurfaceToTexture(generateMapChoiceArrowFrames(pSurface, house).release(), true);
...
std::unique_ptr<Wsafile> GFXManager::loadWsafile(const std::string& filename) const {
    try {
        ...
        auto wsafile = std::make_unique<Wsafile>(file_wsa.get());
        return wsafile;
    } catch (std::exception &e) {
