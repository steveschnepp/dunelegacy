...
Animation* PictureFactory::createSardaukarPlanet(Animation* ordosPlanetAnimation, SDL_Surface* heraldSard) {
    ...
    ordosPlanetAnimation->for_each_frame(
        [&](SDL_Surface* pSurface) {
            auto newFrame = copySurface(pSurface);

            mapColor(newFrame.get(), colorMap);

            { // Scope
                auto newFrameWithoutPlanet = copySurface(pSurface);
            ...
            auto dest = calcDrawingRect(heraldSard, 12, 66);
...
Animation* PictureFactory::createMercenaryPlanet(Animation* atreidesPlanetAnimation, SDL_Surface* heraldMerc) {
    ...
    atreidesPlanetAnimation->for_each_frame(
        [&](SDL_Surface* pSurface) {
            auto newFrame = copySurface(pSurface);
            ...
            SDL_Rect src =  {0, 0, getWidth(heraldMerc), 126};
                auto dest = calcDrawingRect(heraldMerc, 12, 66);
...
Animation* PictureFactory::mapMentatAnimationToFremen(Animation* fremenAnimation) {
    ...
    fremenAnimation->for_each_frame([&](SDL_Surface* pSurface) {
            newAnimation->addFrame(mapMentatSurfaceToFremen(pSurface).release());
        });
...
Animation* PictureFactory::mapMentatAnimationToSardaukar(Animation* harkonnenAnimation) {
    ...
    harkonnenAnimation->for_each_frame(
        [&](SDL_Surface* pSurface) {
            newAnimation->addFrame(mapMentatSurfaceToSardaukar(pSurface).release());
        });
...
Animation* PictureFactory::mapMentatAnimationToMercenary(Animation* ordosAnimation) {
    ...
    ordosAnimation->for_each_frame(
        [&](SDL_Surface* pSurface) {
                newAnimation->addFrame(mapMentatSurfaceToMercenary(pSurface).release());
        });
