...
Icnfile::Icnfile(SDL_RWops* icnRWop, SDL_RWops* mapRWop, int freesrc)
{
    ...
    const auto icnEndOffset = SDL_RWsize(icnRWop);
    if(icnEndOffset <= 0) {
    ...
    const auto mapEndOffset = SDL_RWsize(mapRWop);
    if(mapEndOffset <= 0) {
    ...
    const auto numTilesets = SDL_SwapLE16( *reinterpret_cast<Uint16 *>(pMapFiledata.get()));
    ...
    auto index = SDL_SwapLE16( reinterpret_cast<Uint16*>(pMapFiledata.get())[0]);
    for(int i = 1; i < numTilesets; i++) {
        const auto tmp = SDL_SwapLE16( reinterpret_cast<Uint16*>(pMapFiledata.get())[i]);
...
sdl2::surface_ptr Icnfile::getPicture(Uint32 indexOfFile) const {
    ...
    for(auto y = 0; y < SIZE_Y; ++y) {
        for(auto x = 0; x < SIZE_X; x+=2) {
            auto pixel = filestart[ (y*SIZE_X + x) / 2];
            pixel = pixel >> 4;
...
sdl2::surface_ptr Icnfile::getPictureArray(Uint32 mapfileIndex, int tilesX, int tilesY, int tilesN) {
    ...
    for(auto n = 0; n < tilesN; n++) {
        for(auto tile_y = 0; tile_y < tilesY; ++tile_y) {
            for(auto tile_x = 0; tile_x < tilesX; ++tile_x) {
                const auto IndexOfFile = tilesets[mapfileIndex].tileIndices[tileidx];
                ...
                for(auto y = 0; y < SIZE_Y;y++) {
                    for(auto x = 0; x < SIZE_X; x+=2) {
                        const auto pixel = filestart[ (y*SIZE_X + x) / 2];
...
sdl2::surface_ptr Icnfile::getPictureRow(Uint32 startIndex, Uint32 endIndex, Uint32 maxRowLength) const {
    ...
    auto tileCount = 0u;
    for(auto row = 0u; (row < numRows) && (tileCount < numTiles); ++row) {
        for(auto col = 0u; (col < numCols) && (tileCount < numTiles); ++col) {
            const auto indexOfFile = startIndex + tileCount;
