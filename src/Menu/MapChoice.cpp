...
MapChoice::MapChoice(int newHouse, unsigned int lastMission, Uint32 oldAlreadyPlayedRegions) : MenuBase() {
    ...
    auto numRegions = 0;
    for(int i = 0; i < 4; i++) {
        const auto regionNum = group[lastScenario].attackRegion[i].regionNum;
        if(regionNum > 0) {
            numRegions++;
    ...
    switch(mapChoiceState) {
        ...
        case MAPCHOICESTATE_FADEINPLANET: {
            if(curBlendBlitter == nullptr) {
                auto pSurface = convertSurfaceToDisplayFormat(pGFXManager->getUIGraphicSurface(UI_MapChoicePlanet), false);
            ...
            if(curBlendBlitter != nullptr) {
                const auto numSteps = bFastBlending ? 8 : 1;
        ...
        case MAPCHOICESTATE_BLENDPLANET: {
            if(curBlendBlitter == nullptr) {
                auto pSurface = convertSurfaceToDisplayFormat(pGFXManager->getUIGraphicSurface(UI_MapChoiceMapOnly), false);
                SDL_Rect dest = { 0, 0, getWidth(pSurface.get()), getHeight(pSurface.get()) };
                curBlendBlitter = std::make_unique< BlendBlitter>(pSurface.release(), true, mapSurface.get(), dest);
            }
        ...
        case MAPCHOICESTATE_BLENDMAP: {
            if(curBlendBlitter == nullptr) {
                auto pSurface = convertSurfaceToDisplayFormat(pGFXManager->getUIGraphicSurface(UI_MapChoiceMap), false);
                SDL_Rect dest = { 0, 0, getWidth(pSurface.get()), getHeight(pSurface.get()) };
                curBlendBlitter = std::make_unique<BlendBlitter>(pSurface.release(), true, mapSurface.get(), dest);
            }
        ...
        case MAPCHOICESTATE_BLENDING: {
            if(curBlendBlitter == nullptr) {
                ...
                if((curHouse2Blit < NUM_HOUSES)&&(curRegion2Blit < group[lastScenario].newRegion[(curHouse2Blit + house) % NUM_HOUSES].size())) {
                    ...
                    const auto pieceNum = (group[lastScenario].newRegion[(curHouse2Blit + house) % NUM_HOUSES])[curRegion2Blit];
                    auto pPieceSurface = convertSurfaceToDisplayFormat(pGFXManager->getMapChoicePieceSurface(pieceNum,(curHouse2Blit + house) % NUM_HOUSES), false);
        ...
        case MAPCHOICESTATE_ARROWS:
        {
            ...
            for(auto i = 0; i < 4; i++) {
                ...
                const auto arrowNum = std::max<int>(0, std::min<int>(8, group[lastScenario].attackRegion[i].arrowNum));
                SDL_Texture* arrow = pGFXManager->getUIGraphic(UI_MapChoiceArrow_None + arrowNum, house);
                const auto arrowFrame = (SDL_GetTicks() / 128) % 4;
...
void MapChoice::createMapSurfaceWithPieces(unsigned int scenario) {
    ...
    for(unsigned int s = 1; s < scenario; s++) {
        ...
        for(unsigned int h = 0; h < g.newRegion.size(); h++) {
            for (auto pieceNum : g.newRegion[h]) {
                const auto pieceSurface = pGFXManager->getMapChoicePieceSurface(pieceNum, h);
                auto dest = calcDrawingRect(pieceSurface, piecePosition[pieceNum].x, piecePosition[pieceNum].y);
